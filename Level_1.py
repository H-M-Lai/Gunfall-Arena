...
PLATFORM_RECTS = [
    pygame.Rect(100, 355, 220, 10),
    pygame.Rect(555, 355, 220, 10),
    pygame.Rect(340, 410, 220, 10),
    pygame.Rect(110, 440, 240, 10),
    pygame.Rect(545, 440, 240, 10),
]

_finished = False
_advance_to_next_level = False
_back_rect = pygame.Rect(20, HEIGHT - 68, 140, 44)
_state = {"effects": []}
...
AI_COMBAT_DISTANCE = 100
AI_DISTANCE_TOLERANCE = 40

_game_over = False
_winner = None

# GM2-style physics constants
GROUND_FRICTION = 0.62   # lower = knockback slides further on ground
AIR_FRICTION    = 0.88   # vx multiplier per frame in air (knockback persists longer)
FREEPASS_MS     = 380    # invincibility window after being hit
...
# Adjust recoil wind streak size and fade here.
class WindEffect:
...
WEAPON_PROFILES = {
    # blowback = horizontal force overwritten onto target when hit
    # pushback = backward self-recoil applied to shooter
...
def _load_level_assets():
    ...
    global BG_IMAGE, PLATFORM_IMAGE

    if BG_IMAGE is None:
        BG_IMAGE = pygame.image.load(
            os.path.join(ASSET_DIR, "background", "level1.jpeg")
        ).convert()

    if PLATFORM_IMAGE is None:
        PLATFORM_IMAGE = pygame.transform.smoothscale(
            pygame.image.load(
                os.path.join(ASSET_DIR, "background", "platform.png")
            ).convert_alpha(),
            (780, 420),
        )
...
class _Player:
    ...
    def __init__(self, x, label, controls, frames, gender="female", team="ally", is_ai=False):
        self.label = label
        ...
        self._set_weapon_frames(frames[:] if frames else [self._fallback()])
        self.image = self.stand_right

        self.spawn_x = x
        self.spawn_y = PLATFORM_RECTS[2].top
        ...
        # target commitment (targettime) - AI locks onto one enemy for a duration
        self.ai_target = None
        self.ai_target_until = 0
        # optimalx - preferred fighting X position (offset from target)
        self.ai_optimal_x = x
        self.ai_optimal_until = 0
        # stuck detection (idletime2 + prevx from GM2)
        self.ai_prev_x = x
        self.ai_idle_ticks = 0
        # resting state - brief pause after repositioning
        self.ai_resting = False
        self.ai_rest_until = 0
        # simulated keypresses (GM2 style: same physics runs for AI and human)
    ...
    def _fallback(self):
        ...
        s = pygame.Surface((48, 70), pygame.SRCALPHA)
        s.fill((120, 180, 230))
    ...
    def _rebuild_scaled_frames(self):
        if not self.base_walk_right:
            self.base_walk_right = [self._fallback()]
    ...
    def update(self, keys, players=None, bullets=None):
        ...
        # Key input will override vx; when no key is held knockback slides to a stop
        if self.on_ground:
            self.vx *= GROUND_FRICTION
        else:
            self.vx *= AIR_FRICTION
        # Snap tiny residual velocity to zero to avoid perpetual micro-sliding
        if abs(self.vx) < 0.3:
            self.vx = 0.0
        ...
        if self.rect.top > HEIGHT:
            ...
            self.lives -= 1
            if self.lives <= 0:
                self.rect.midbottom = (-200, HEIGHT + 200)
                self.vx = 0
                self.vy = 0
                self.on_ground = False
                self.current_platform_idx = None
    ...
    def draw(self, screen):
        ...
        name = NAME_FONT.render(self.label, True, (255, 255, 255))
        name_rect = name.get_rect(center=(self.rect.centerx, self.rect.top + 30))
        ...
        if self.reloading_until > now and self.reload_time_ms > 0:
            ...
            ring_center = (name_rect.right + 14, name_rect.centery)
            ring_radius = 7
    ...
    def _choose_ai_loot_target(self):
        ...
        for drop in drops:
            ...
            # Prefer useful nearby pickups without abandoning fights across the map.
            score = -(dx * 0.7 + dy * 1.1)
    ...
    def _run_ai_brain(self, now, players, bullets):
        ...
        # ── 1. TARGET COMMITMENT (targettime) ──────────────────────────────
        # Lock onto a target for a random duration before re-evaluating
        ...
        # Recovery: if knocked off and falling, use an air jump to climb back to a nearby platform.
        ...
        if loot_target is not None:
            ...
            # While chasing loot, stand directly on its X so pickup collision can trigger.
        ...
        # ── 3. OPTIMAL X (GM2's optimalx) ─────────────────────────────────
        # Recalculate preferred position periodically
        if now > self.ai_optimal_until:
        ...
        if self.ai_idle_ticks > 25 and self.on_ground:
            # Stuck — recalculate a new optimal_x rather than blindly jumping off the edge
            self.ai_optimal_until = 0   # force recalculate next frame
            self.ai_idle_ticks = 0
            # Only jump if there's safe ground in BOTH directions (truly walled in)
            if ground_left and ground_right and now >= self.ai_next_jump_time:
                self.ai_key_jump = True
                self.ai_next_jump_time = now + 600
            return

        # ── 6. MOVE TOWARD OPTIMAL X ──────────────────────────────────────
        gap = self.rect.centerx - self.ai_optimal_x
        if abs(gap) > 12:
            want_right = gap < 0
            # Edge safety: only move if ground probe confirms solid floor ahead.
            # NEVER jump because of an edge — that sends the bot off the platform.
            if want_right:
                if ground_right:
                    self.ai_key_right = True
                else:
                    # Re-plan away from edge so enemy cannot "range-push" indefinitely.
                    self.ai_optimal_until = 0
                    self.ai_resting = True
                    self.ai_rest_until = now + 220
            else:
                if ground_left:
                    self.ai_key_left = True
                else:
        ...
        # If target is too high/low to shoot, force level transition to regain shooting line.
        ...
        if incoming and now >= self.ai_next_jump_time:
    ...
    def _probe_ground(self, probe_x):
        ...
        Uses a generous vertical window so it stays reliable while slightly airborne."""
        return any(
    ...
    def _spawn_burst(self, direction, bullets, aim_vy=0.0):
        ...
        if self.on_ground and self.current_platform_idx is not None:
            platform_top = PLATFORM_RECTS[self.current_platform_idx].top
            muzzle_y = min(muzzle_y, platform_top - 8)
        # Recoil belongs to shooting itself, not hit-confirm.
        self.vx += -direction * self.pushback_force
        # blowback_force = target knockback; small upward nudge keeps feel without forced jump
        kb_y = -2.5
...
def _legacy_level1_ai(self, now, players, bullets):
    self.ai_key_left = False
    self.ai_key_right = False
    self.ai_key_jump = False
    self.ai_key_down = False
    ...
    loot = self._choose_ai_loot_target()

    if loot:
        dx_loot = loot.rect.centerx - self.rect.centerx
        dy_loot = loot.rect.centery - self.rect.centery

        if abs(dx_loot) > 6:
            if dx_loot > 0:
                if self._probe_ground(self.rect.centerx + 20):
                    self.ai_key_right = True
                elif self._has_safe_drop_below():
                    self.ai_key_down = True
            else:
                if self._probe_ground(self.rect.centerx - 20):
                    self.ai_key_left = True
        ...
        if dy_loot < -40 and self.jumps_remaining > 0:
            if now >= self.ai_next_jump_time:
                self.ai_key_jump = True
                self.ai_next_jump_time = now + 400

        if dy_loot > 50 and self.on_ground:
            if self._has_safe_drop_below():
                self.ai_key_down = True
    ...
    for b in bullets:
        ...
        if abs(b.y - self.rect.centery) < 30:
            if b.vx > 0 and b.x < self.rect.centerx:
                if self.rect.centerx - b.x < 140:
                    if self.on_ground and self._has_safe_drop_below():
                        self.ai_key_down = True
                        return
    ...
    if dist > AI_COMBAT_DISTANCE + AI_DISTANCE_TOLERANCE:
        if dx > 0:
            if self._probe_ground(self.rect.centerx + 20):
                self.ai_key_right = True
            elif self.jumps_remaining > 0:
    ...
    elif dist < AI_COMBAT_DISTANCE - AI_DISTANCE_TOLERANCE:
        if dx > 0:
            if self._probe_ground(self.rect.centerx - 20):
                self.ai_key_left = True
        else:
    ...
    if self.jumps_remaining > 0 and now >= self.ai_next_jump_time:
        if self.on_ground and dy < -40:
            self.ai_key_jump = True
            self.ai_next_jump_time = now + 420
        elif not self.on_ground and dy < -20:
            self.ai_key_jump = True
            self.ai_next_jump_time = now + 260
...
_Player._run_ai_brain = _legacy_level1_ai


def _refresh_game_over_state():
    global _game_over, _winner
    players = _state.get("players", [])
    alive_players = [p for p in players if p.lives > 0 and not p.is_ai]
    alive_ai = [p for p in players if p.lives > 0 and p.is_ai]
    if not _game_over:
...
def _draw_hud(screen):
    ...
    for i, player in enumerate(players):
        ...
        heart_x = x + box_width - 18 - (max(0, player.lives - 1) * 13)

        for h in range(player.lives):

            pygame.draw.circle(screen, (230, 40, 40), (heart_x + h * 13, y + 13), 5)

        weapon_text = info_font.render(
            f"Wpn: {player.weapon}", True, (200, 200, 255)
        )
...
def _visible_player_rect(player):
    vis = player.image.get_bounding_rect()
...
class _Bullet:
    ...
    def draw(self, screen):
        ...
        # Compute tail position along velocity direction
        ...
        # Perpendicular for the polygon width
        perp_x = -norm_vy
        perp_y = norm_vx
...
def _update_bullets():
    ...
    for bullet in bullets[:]:
        ...
        if hit_player is not None:
            ...
            # Skip if target is still invincible (spawn shield or freepass).
...
def reset(
    ...
    global _state, drop_manager, _finished, _game_over, _winner, _round_start_tick
    global _advance_to_next_level
    global _paused
    ...
    p1_x, p1_y = _random_spawn_point()
    p1 = _Player(p1_x, player1, controls_p1, p1_frames, p1_gender, team="ally", is_ai=False)
    p1.spawn_x, p1.spawn_y = p1_x, p1_y
    p1.rect.midbottom = (p1_x, p1_y)
    ...
    _state = {"mode": mode, "players": [p1], "bullets": [], "effects": []}
    now = pygame.time.get_ticks()
    ...
    if mode == "2P":
        p2_x, p2_y = _random_spawn_point([p1_x])
        p2 = _Player(p2_x, player2, controls_p2.copy(), p2_frames, p2_gender, team="ally", is_ai=False)
        p2.spawn_x, p2.spawn_y = p2_x, p2_y
        p2.rect.midbottom = (p2_x, p2_y)
    ...
    ai_count = 2 if mode == "2P" else 1
    for idx in range(ai_count):
        ai_x, ai_y = _random_spawn_point(taken_x)
        snowman = _Player(
            ai_x,
            f"Snowman {idx + 1}" if ai_count > 1 else "Snowman",
            {},
            snow_frames,
            "snowman",
            team="enemy",
        ...
        snowman.spawn_x, snowman.spawn_y = ai_x, ai_y
        snowman.rect.midbottom = (ai_x, ai_y)
    ...
    drop_manager = DropManager(PLATFORM_RECTS)
...
def draw(screen, title_font, menu_font, small_font):
    ...
    if _game_over:
        ...
        title_font_big = pygame.font.SysFont("arial", 95, True)
        subtitle_font = pygame.font.SysFont("arial", 32, True)
        ...
        if _winner == "Snowman":
            ...
            subtitle = subtitle_font.render(
                "The Village Has Fallen...", True, (255, 255, 255)
            )
        else:
            title = title_font_big.render("VICTORY", True, (255, 215, 0))
            subtitle = subtitle_font.render(
                "The Village Is Safe", True, (255, 255, 255)
            )
        ...
        for idx, p in enumerate(players_to_show):
            ...
            acc = 0
            if p.shots_fired > 0:
                acc = int((p.shots_hit / p.shots_fired) * 100)
            ...
            screen.blit(
                name,
                name.get_rect(center=(panel_x + panel_w // 2, panel_y + 30)),
            )
            ...
            for i, (label, value) in enumerate(stats):
                ...
                screen.blit(label_text, (panel_x + 50, y - 15))
                screen.blit(value_text, (panel_x + panel_w - 60, y - 15))

        if _winner != "Snowman":
            hint_text = "ENTER : Next Level     ESC : Level Select"
        else:
            hint_text = "Press ESC to return to level select"

        hint = hint_font.render(hint_text, True, (180, 180, 180))
        screen.blit(hint, hint.get_rect(center=(WIDTH // 2, panel_y + panel_h + 40)))
...
def consume_advance_to_next_level():
    global _advance_to_next_level
    value = _advance_to_next_level
    _advance_to_next_level = False
    return value
...
def is_hovering_clickable(pos):
    ...
    return _back_rect.collidepoint(pos)
...
def handle_key(key):
    ...
    global _finished, _paused, _advance_to_next_level

    if key == pygame.K_RETURN and _game_over and _winner != "Snowman":
        _stop_walk_sfx()
        _advance_to_next_level = True
        _finished = True
        return
